...
MAP_NAME = 'FalconEasy' #'Name of the map for trajectory generation.'
PLAYER_NAME = 'Player'
FULL_OBS = True
...
if __name__ == '__main__':
    ...
    if NUM_TRAJECTORIES == 0 or MAP_NAME not in default_maps:
        ...
        while step <= TRAJ_LENGTH:
            ...
            if step == 0:
                player_pre_utility = agent.getState("__REWARD__").domain()[0]
            ...
            traj_world = trajectories[step][0][0]
            traj_agent = traj_world.agents[agent.name]
            logging.info(f"Action: {trajectories[step][0][1]}")
            ...
            logging.info(f"CURUTILITY: {player_cur_utility}")
            step = step + 1
        pass
