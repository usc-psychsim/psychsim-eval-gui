...
if __name__ == '__main__':
    ...
    if args.trajectories == 0 or args.map_name not in default_maps:
        ...
        for t in trajectories[0]:
            ...
            loc = traj_world.getFeature(f"{agent.name}'s loc", traj_world.state)
            reward = traj_agent.getState("__REWARD__").domain()[0]
        ...
        pickle_out_name = os.path.join(args.output, f"trajectories_pickle{timestr}")
        logging.info(f"saving file: {pickle_out_name}")
